...
class WeatherAgent:
    def __init__(self) -> None:
        ...
        # sessionId -> list of message dicts (conversation history, excluding system prompt)
    ...
    async def run(self, session_id: str, user_text: str) -> str:
        ...
        # Agentic loop: call LLM → handle tool calls → repeat until final answer
        while True:
            ...
            # Append assistant turn (convert to dict, dropping None fields)
    ...
    async def run_streaming(self, session_id: str, user_text: str):
        ...
        Tool calls are resolved non-streaming; only the final answer is streamed.
        """
        history = self._get_history(session_id)
        history.append({"role": "user", "content": user_text})
        ...
        while True:
            ...
            if not message.tool_calls:
                break  # ready to stream the final answer
        ...
        # Stream the final response (no tools needed at this stage)
        full_reply = ""
