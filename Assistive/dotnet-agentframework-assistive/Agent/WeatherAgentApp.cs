...
namespace WeatherAgentAssistive.Agent;

public class WeatherAgentApp : AgentApplication
{
    ...
    private readonly ILogger<WeatherAgentApp> _logger;
    private readonly IChatClient _chatClient;
    private readonly WeatherLookupTool _weatherTool;

    public WeatherAgentApp(
        AgentApplicationOptions options,
        IChatClient chatClient,
        WeatherLookupTool weatherTool,
        ILogger<WeatherAgentApp> logger) : base(options)
    {
        _logger = logger;
        ...
        foreach (var member in tc.Activity.MembersAdded)
        {
            if (member.Id != tc.Activity.Recipient.Id)
            {
                await tc.SendActivityAsync(
                    MessageFactory.Text("Hello! I am your weather assistant. Ask me about weather conditions in any city!"),
                    ct);
            }
        }
    }
    ...
    private Task<string> GetCurrentWeather(
        [Description("The name of the city, e.g. 'Chennai', 'Seattle', 'London'")] string city)
        => _weatherTool.GetCurrentWeather(city);
